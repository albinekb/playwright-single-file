...
async function getScriptSource(options) {
  let source = ''
  source += script
  source += await readScriptFiles(
    options && options.browserScripts ? options.browserScripts : [],
  )
  if (options.browserStylesheets && options.browserStylesheets.length) {
    source +=
      'addEventListener("load",()=>{const styleElement=document.createElement("style");styleElement.textContent=' +
      JSON.stringify(await readScriptFiles(options.browserStylesheets)) +
